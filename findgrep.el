;;; findgrep.el --- An util for advanced recursive search  -*- lexical-binding: t -*-

;; Author: kevwargo
;; Package-Requires: ((emacs "30.1") transient s dash grep)

;;; Commentary:

;; FindGrep is used for searching directories by using a transient-based
;; interface for quick and rich customization of search patterns and filters.
...
(cl-defmethod transient-format-description :around ((obj findgrep-switch))
  (let ((str (cl-call-next-method obj))
        (arg (oref obj argument)))
    (when (s-prefix? "--no-" arg)
      (add-face-text-property 0 (length str) '(:background "#d8ffff") nil str))
    str))
